...
def process_items(items: List[Dict[str, Any]], file_name: str, index_name: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # 批量生成向量；本地 demo 没有 API key 时仍保留关键词/全文检索能力。
        ...
        for item, embedding in zip(items, embeddings):
            ...
            # 将嵌入向量存储到字典中；无向量时 Elasticsearch 仍可走关键词召回。
            if embedding:
...
def execute_insert_process(file_path: str, file_name: str, index_name: str):
    ...
    if not documents:
        ...
        return {
            ...
            "processed": 0,
            "indexed": 0,
            "errors": [f"No documents found in {file_path}"],
        }
    ...
    if not processed_documents:
        ...
        return {
            ...
            "processed": 0,
            "indexed": 0,
            "errors": [f"Failed to process documents from {file_path}"],
        }
    ...
    try:
        ...
        try:
            es_connection.es.indices.refresh(index=index_name)
        except Exception:
            pass
        indexed = len(processed_documents) - len(errors)
